    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 pt-20 dark:from-primary-black dark:via-gray-900 dark:to-primary-black">
      <article className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <nav className="mb-8 text-sm text-gray-500 dark:text-gray-400">
          <Link href={`/${locale}/blog`} className="hover:text-primary-orange">{locale === "fa" ? "وبلاگ" : locale === "ar" ? "المدونة" : "Blog"}</Link>
          <span className="mx-2">/</span>
          <span>{article.title}</span>
        </nav>

        <header className="mb-10">
          {article.category && <p className="mb-3 font-medium text-primary-orange">{article.category.name}</p>}
          <h1 className="text-3xl font-bold leading-tight text-gray-900 dark:text-white md:text-5xl">{article.title}</h1>
          <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">{date}</p>
          {article.excerpt && <p className="mt-6 text-xl leading-9 text-gray-600 dark:text-gray-300">{article.excerpt}</p>}
        </header>

        {featuredImage && (
          <div className="relative mb-10 aspect-video overflow-hidden rounded-3xl bg-gray-200 dark:bg-gray-800">
            <Image src={featuredImage} alt={article.title} fill priority className="object-cover" sizes="(max-width: 896px) 100vw, 896px" />
          </div>
        )}

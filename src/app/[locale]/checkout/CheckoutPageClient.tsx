...
export default function CheckoutPageClient({
  ...
  useEffect(() => {
    ...
    if (orderNumber && refId && !error) {
      router.replace(
        `/${locale}/checkout/success?orderNumber=${orderNumber}&refId=${refId}`,
      );
      return;
    }
    ...
    if (error && orderNumber) {
      ...
      switch (error) {
        case "payment_cancelled":
          errorText = String(
            messages?.checkout?.paymentCancelled || "پرداخت لغو شد",
          );
          break;
        case "payment_failed":
        ...
        case "payment_request_failed":
          errorText = String(
            messages?.checkout?.paymentRequestFailed ||
              "خطا در ایجاد درخواست پرداخت",
          );
          break;
        case "order_not_found":
          errorText = String(
            messages?.checkout?.orderNotFound || "سفارش یافت نشد",
          );
          break;
        default:
          errorText = String(
            messages?.checkout?.paymentError || "خطا در پرداخت",
          );
      }
    ...
    const fetchShippingMethods = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          const transformed = (result.data as ApiShippingMethod[]).map(
            (method) => ({
              id: method.id,
              name: method.name,
  ...
  const shippingCost = isRetryMode
    ? failedOrder.shippingAmount
    : selectedShippingMethod?.price || 0;
  const taxAmount = isRetryMode
    ? failedOrder.taxAmount
    : Math.round(totalPrice * 0.09);
  const discountAmount = isRetryMode
    ? failedOrder.discountAmount
    : appliedCoupon?.discountAmount || 0;
  const totalWithShipping = isRetryMode
    ? failedOrder.totalAmount
    : totalPrice + shippingCost + taxAmount - discountAmount;
    ...
    if (currentStep === 1) {
      ...
      if (useSavedAddresses) {
        if (!selectedShippingAddress) {
          alert(
            String(
              t.shippingAddressRequired || "لطفاً آدرس ارسال را انتخاب کنید",
            ),
          );
          return;
        }
        ...
        const requiredMessage = String(
          messages.common?.requiredField || t.completeAddressFields,
        );
        const missingFields = (
        ...
        if (missingFields.length) {
          setAddressErrors(
            Object.fromEntries(
              missingFields.map((field) => [field, requiredMessage]),
            ),
          );
          requestAnimationFrame(() => {
            document
              .querySelector<HTMLElement>(
                `[data-address-field="${missingFields[0]}"]`,
              )
              ?.focus();
          });
          return;
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          paymentResult.error ||
            String(
              t.paymentRequestFailed || "Failed to create payment request",
            ),
        );
      }
      ...
      alert(
        `${String(t.orderError)}: ${error instanceof Error ? error.message : String(t.tryAgain)}`,
      );
    } finally {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        ...
        if (
          response.status === 404 &&
          result.error?.includes("User account not found")
        ) {
          const shouldLogout = window.confirm(
            String(
              t.userAccountNotFound ||
                "Your account was not found. Please log out and log in again. Would you like to log out now?",
            ),
          );
          if (shouldLogout) {
      ...
      if (orderData.paymentMethod === "ZARINPAL") {
        ...
        if (
          ...
          await fetch(`/api/customer/orders/${result.data.id}`, {
            ...
            console.error(
              "Failed to cancel unpaid order after payment request failure:",
              cancelError,
            );
          });
          throw new Error(
      ...
      clearCart();
      router.push(
        `/${locale}/checkout/success?orderNumber=${result.data.orderNumber}`,
      );
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {isRetryMode && failedOrder && (
          <div className="mb-6 p-6 bg-blue-50 dark:bg-blue-500/10 border-2 border-blue-200 dark:border-blue-500/30 rounded-xl">
            <div className="flex items-start">
              <svg
                className="w-6 h-6 text-blue-600 dark:text-blue-400 mt-0.5 ml-2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        {paymentError && (
          <div className="mb-6 p-4 bg-red-50 dark:bg-red-500/10 border-2 border-red-200 dark:border-red-500/30 rounded-xl">
            <div className="flex items-start">
              <svg
                className="w-6 h-6 text-red-600 dark:text-red-400 mt-0.5 ml-2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div className="flex-1">
                <h3 className="text-red-800 dark:text-red-300 font-semibold mb-1">
                  {String(
                    messages?.checkout?.paymentErrorTitle || "خطا در پرداخت",
                  )}
                </h3>
                ...
                {step < 3 && (
                  <div
                    className={`w-16 h-1 mx-4 ${
                      currentStep > step
                        ? "bg-primary-orange"
                        : "bg-gray-200 dark:bg-gray-50 dark:bg-white/10"
                    }`}
                  />
                )}
              </div>
                ...
                {isRetryMode && failedOrder ? (
                  <div className="p-6 bg-blue-50 dark:bg-blue-500/10 border-2 border-blue-200 dark:border-blue-500/30 rounded-xl">
                    <div className="flex items-start mb-4">
                      <svg
                        className="w-5 h-5 text-blue-600 dark:text-blue-400 mt-0.5 ml-2"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      <p className="text-blue-700 dark:text-blue-400 text-sm">
                        آدرس ارسال از سفارش قبلی شما استفاده می‌شود. برای تغییر
                        آدرس، لطفاً سفارش را لغو کرده و مجدداً ثبت کنید.
                      </p>
                    </div>
                    <div className="text-gray-900 dark:text-white">
                      ...
                      <p>
                        ...
                        {failedOrder.shippingAddress.lastName}
                      </p>
                      <p>{failedOrder.shippingAddress.phone}</p>
                      <p>
                        {failedOrder.shippingAddress.state}،{" "}
                        {failedOrder.shippingAddress.city}
                      </p>
              ...
              <div className="readable-panel rounded-3xl p-6 sm:p-8">
                ...
                {shippingMethodsLoading ? (
                  ...
                  <div className="text-center py-12 text-gray-600 dark:text-gray-400">
                    {String(
                      t.noShippingMethods || "هیچ روش ارسالی در دسترس نیست",
                    )}
                  </div>
                ) : (
              ...
              <div className="readable-panel rounded-3xl p-6 sm:p-8">
                ...
                <div className="mb-8 rounded-2xl border border-gray-200 bg-gray-50 p-6 dark:border-white/10 dark:bg-gray-800/80">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                    {String(
                      t.shippingInfoLabel || t.shippingAddress || "آدرس ارسال",
                    )}
                  </h3>
                  <div className="text-gray-600 dark:text-gray-300">
                    {useSavedAddresses && selectedShippingAddress ? (
                      ...
                      <>
                        ...
                        <p>{address.address}</p>
                        <p>
                          {String(t.postalCodeLabel)}: {address.postalCode}
                        </p>
                      </>
                    )}
                  </div>
                ...
                <div className="mb-8 rounded-2xl border border-gray-200 bg-gray-50 p-6 dark:border-white/10 dark:bg-gray-800/80">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                    {String(t.shippingMethodLabel)}
                  </h3>
                  <div className="text-gray-600 dark:text-gray-300">
                    <p>{selectedShippingMethod?.name}</p>
                    <p>
                      {String(t.deliveryTime)}:{" "}
                      {selectedShippingMethod?.estimatedDays}
                    </p>
                    <p>
                      {String(t.shippingCost)}:{" "}
                      {formatPrice(selectedShippingMethod?.price || 0)}
                    </p>
                  </div>
                </div>

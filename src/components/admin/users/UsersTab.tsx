...
export default function UsersTab() {
      ...
      if (result.success) {
        ...
        setStats({
          ...
          newUsersThisMonth: allUsers.filter(
            (u: AdminUser) => new Date(u.createdAt) >= thisMonth,
          ).length,
  ...
  const handleUserUpdate = async (data: UpdateUserData) => {
    try {
      ...
      alert(
        `خطا در بروزرسانی کاربر: ${error instanceof Error ? error.message : "خطای شبکه"}`,
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const handleStatusChange = async (userId: string, isActive: boolean) => {
    try {
      ...
      alert(
        `خطا در تغییر وضعیت کاربر: ${error instanceof Error ? error.message : "خطای شبکه"}`,
      );
    }
  };
  ...
  const handleRoleChange = async (userId: string, role: UserRole) => {
    try {
      ...
      alert(
        `خطا در تغییر نقش کاربر: ${error instanceof Error ? error.message : "خطای شبکه"}`,
      );
    }
  };
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      alert(
        `خطا در حذف کاربر: ${error instanceof Error ? error.message : "خطای شبکه"}`,
      );
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">{copy.totalUsers}</p>
        ...
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">{copy.activeUsers}</p>
              <p className="text-3xl font-bold text-white">
                {stats.activeUsers}
              </p>
            </div>
        ...
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">{copy.newUsersThisMonth}</p>
              <p className="text-3xl font-bold text-white">
        ...
        <div className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">{copy.adminUsers}</p>
              <p className="text-3xl font-bold text-white">
                {stats.adminUsers}
              </p>
            </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0 lg:space-x-6">
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            >
              ...
              <option value="SUPER_ADMIN">{copy.superAdmin}</option>
            </select>
      ...
      {isLoading ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">{copy.loadingUsers}</p>
        </div>
      ) : (
        <UserList
          users={users}

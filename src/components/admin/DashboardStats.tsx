  ...
  const StatCard = ({ title, value, change, icon, color }: StatCardProps) => (
    <div className="glass rounded-3xl p-4 sm:p-6 hover:scale-105 transition-transform duration-300">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div
          className={`w-10 h-10 sm:w-12 sm:h-12 ${color} rounded-2xl flex items-center justify-center`}
        >
          {icon}
        </div>
        <div
          className={`text-right ${change >= 0 ? "text-green-400" : "text-red-400"} text-xs sm:text-sm`}
        >
      ...
      <div className="text-center">
        <h3 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">
          {isLoading ? (
            ...
            formatPrice(value as number, locale)
          ) : (

...
export default function RecentOrders({ locale }: RecentOrdersProps) {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "/api/orders?limit=5&sortBy=createdAt&sortOrder=desc",
          {
        ...
        if (
          ...
          const ordersData = result.data.data.map(
            (order: {
              id: string;
              orderNumber: string;
  ...
  const getStatusInfo = (status: string) => {
    const statusMap = {
      pending: {
        label: String(t.statusPending),
        color: "bg-yellow-500/20 text-yellow-400",
      },
      processing: {
        label: String(t.statusProcessing),
        color: "bg-blue-500/20 text-blue-400",
      },
      shipped: {
        label: String(t.statusShipped),
        color: "bg-purple-500/20 text-purple-400",
      },
      delivered: {
        label: String(t.statusDelivered),
        color: "bg-green-500/20 text-green-400",
      },
      cancelled: {
        label: String(t.statusCancelled),
